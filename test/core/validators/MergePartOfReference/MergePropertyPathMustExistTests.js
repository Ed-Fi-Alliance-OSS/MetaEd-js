...
describe('MergePropertyPathMustExist', () => {
    ...
    let propertyPathLookup = MockRepository.GenerateStub();
    propertyPathLookup.Stub(x => x.Validate(Arg(Matches(y => y.Name == "Entity2"), Arg.Is(new Array("Entity1", "Why")), Arg(Is.NotNull)).Return(false)));
    let validatorListener = new ValidatorListener_1.ValidatorListener(new TestRuleProvider_1.TestRuleProvider(new MergePropertyPathMustExist(helper.symbolTable, this._propertyPathLookup)));
    it('should_have_validation_failures()', () => {
        helper.errorMessageCollection.Count.ShouldEqual(1);
    });
    it('should_have_meaningful_validation_message()', () => {
        helper.errorMessageCollection[0].Message.ShouldEqual("Path Entity1.Why is not valid.");
    });
});
...
let propertyPathLookup = MockRepository.GenerateStub();
propertyPathLookup.Stub(x => x.Validate(Arg(Matches(y => y.Name == "Entity2"), Arg.Is(new Array("Entity1", "Prop1")), Arg(Is.NotNull)).Return(true)));
let validatorListener = new ValidatorListener_1.ValidatorListener(new TestRuleProvider_1.TestRuleProvider(new MergePropertyPathMustExist(helper.symbolTable, this._propertyPathLookup)));
...
let propertyPathLookup = MockRepository.GenerateStub();
propertyPathLookup.Stub(x => x.Validate(Arg(Matches(y => y.Name == "Entity3"), Arg.Is(new Array("Entity1", "Prop1")), Arg(Is.NotNull)).Return(true)));
let validatorListener = new ValidatorListener_1.ValidatorListener(new TestRuleProvider_1.TestRuleProvider(new MergePropertyPathMustExist(helper.symbolTable, this._propertyPathLookup)));
it('should_have_no_validation_failures()', () => {
    helper.errorMessageCollection.Count.ShouldEqual(0);
});
...
let propertyPathLookup = MockRepository.GenerateStub();
propertyPathLookup.Stub(x => x.Validate(Arg(Matches(y => y.Name == "Entity4"), Arg.Is(new Array("Entity1", "Prop1")), Arg(Is.NotNull)).Return(true)));
let validatorListener = new ValidatorListener_1.ValidatorListener(new TestRuleProvider_1.TestRuleProvider(new MergePropertyPathMustExist(helper.symbolTable, this._propertyPathLookup)));
it('should_have_no_validation_failures()', () => {
    helper.errorMessageCollection.Count.ShouldEqual(0);
});
...
let propertyPathLookup = MockRepository.GenerateStub();
propertyPathLookup.Stub(x => x.Validate(Arg(Matches(y => y.Name == "Entity5"), Arg.Is(new Array("Entity1", "Prop1")), Arg(Is.NotNull)).Return(true)));
let validatorListener = new ValidatorListener_1.ValidatorListener(new TestRuleProvider_1.TestRuleProvider(new MergePropertyPathMustExist(helper.symbolTable, this._propertyPathLookup)));
it('should_have_no_validation_failures()', () => {
    helper.errorMessageCollection.Count.ShouldEqual(0);
});
;
describe('When_abstract_entity_has_merge_property', () => { }, private, _propertyPathLookup, IPropertyPathLookup);
let helper = new ValidationTestHelper_1.ValidationTestHelper();
before(() => {
    let metaEdText = MetaEdTextBuilder_1.default.buildIt
        .withBeginNamespace("edfi")
