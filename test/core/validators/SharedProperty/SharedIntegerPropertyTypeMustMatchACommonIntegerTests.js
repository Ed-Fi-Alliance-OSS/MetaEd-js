...
describe('SharedIntegerPropertyTypeMustMatchACommonIntegerTests', () => {
  ...
  describe('When_shared_integer_property_has_invalid_type_and_no_identifier', () => {
    const entityName: string = 'DoesNotExist';
    const helper: ValidatorTestHelper = new ValidatorTestHelper();
    before(() => {
      const metaEdText = MetaEdTextBuilder.build()
        ...
        .withDocumentation('doc')
        .withStringIdentity('RequirePrimaryKey', 'doc', 100)
        .withSharedIntegerProperty(entityName, null, 'doc', true, false)
        .withEndDomainEntity()
        .withEndNamespace()
        .toString();
      helper.setup(metaEdText, validatorListener);
