...
const DS52_FILE = path.join(__dirname, 'packages/metaed-plugin-edfi-api-schema/test/integration/artifact/v7_3/ds-5.2-api-schema-generated.json');
...
function compareParameters(param1, param2, paramName) {
  const differences = [];
  ...
  // Compare schema (ignoring property order)
  if (!areObjectsEqual(param1.schema, param2.schema)) {
    differences.push({
      field: 'schema',
  ...
  // Compare x-Ed-Fi-isIdentity
  if (param1['x-Ed-Fi-isIdentity'] !== param2['x-Ed-Fi-isIdentity']) {
    differences.push({
...
function isDescriptorEndpoint(resourceName) {
  return resourceName.toLowerCase().endsWith('descriptors') || 
         resourceName.toLowerCase().includes('descriptor');
}
...
function compareOpenApiParameters() {
  ...
  console.log(`${colors.yellow}Ignoring parameters: ${Array.from(IGNORED_PARAMETERS).filter(p => p !== 'minChangeVersion' && p !== 'maxChangeVersion').join(', ')}${colors.reset}`);
  ...
  nonDescriptorResources.forEach(resourceName => {
    ...
    swaggerParamMap.forEach((swaggerParam, name) => {
      if (ds52ParamMap.has(name)) {
        ...
        const paramDiffs = compareParameters(swaggerParam, ds52Param, name);
    ...
    if (differences.onlyInSwagger.length > 0 || 
      ...
      totalDifferences += differences.onlyInSwagger.length + 
  ...
  // Example: Show specific comparison for disciplineActions
  console.log(`\n${colors.bright}${colors.cyan}Example: disciplineActions endpoint comparison${colors.reset}`);
  console.log(`${colors.gray}${'='.repeat(50)}${colors.reset}\n`);
  
  const disciplineActionsResult = comparisonResults.find(r => r.resource === 'disciplineActions');
  
  if (disciplineActionsResult) {
    console.log(`${colors.blue}Parameter Differences:${colors.reset}`);
    console.log(`  Parameters only in swagger: ${disciplineActionsResult.onlyInSwagger.map(p => p.name).join(', ') || '(none)'}`);
    console.log(`  Parameters only in ds-5.2: ${disciplineActionsResult.onlyInDs52.map(p => p.name).join(', ') || '(none)'}`);
    
    if (disciplineActionsResult.parameterDifferences.length > 0) {
      console.log(`  Parameters with differences: ${disciplineActionsResult.parameterDifferences.map(p => p.name).join(', ')}`);
    }
    
    console.log(`\n${colors.blue}Query Field Differences:${colors.reset}`);
    console.log(`  Query fields only in swagger: ${disciplineActionsResult.queryFieldsOnlyInSwagger.join(', ') || '(none)'}`);
    console.log(`  Query fields only in ds-5.2: ${disciplineActionsResult.queryFieldsOnlyInDs52.join(', ') || '(none)'}`);
    
    // Get line numbers for reference
    const swaggerContent = fs.readFileSync(SWAGGER_FILE, 'utf8');
    const ds52Content = fs.readFileSync(DS52_FILE, 'utf8');
    
    const swaggerLines = swaggerContent.split('\n');
    const ds52Lines = ds52Content.split('\n');
    
    // Find line numbers for disciplineActions parameters
    let swaggerLineNum = 0;
    let ds52LineNum = 0;
    
    for (let i = 0; i < swaggerLines.length; i++) {
      if (swaggerLines[i].includes('"/ed-fi/disciplineActions"') && swaggerLines[i+1]?.includes('get')) {
        for (let j = i; j < Math.min(i + 50, swaggerLines.length); j++) {
          if (swaggerLines[j].includes('"parameters"')) {
            swaggerLineNum = j + 1;
            break;
          }
        }
    ...
    for (let i = 0; i < ds52Lines.length; i++) {
      if (ds52Lines[i].includes('"/ed-fi/disciplineActions"') && ds52Lines[i+1]?.includes('get')) {
        for (let j = i; j < Math.min(i + 50, ds52Lines.length); j++) {
          if (ds52Lines[j].includes('"parameters"')) {
            ds52LineNum = j + 1;
            break;
          }
        }
    ...
    console.log(`\n  ${colors.gray}Reference line numbers:${colors.reset}`);
    console.log(`    swagger parameters: line ${swaggerLineNum || 'not found'}`);
    console.log(`    ds-5.2 openApiFragments: line ${ds52LineNum || 'not found'}`);
    console.log(`    ds-5.2 queryFieldMapping: line 61507`);
  } else {
    console.log(`  No differences found for disciplineActions endpoint.`);
  }
}
