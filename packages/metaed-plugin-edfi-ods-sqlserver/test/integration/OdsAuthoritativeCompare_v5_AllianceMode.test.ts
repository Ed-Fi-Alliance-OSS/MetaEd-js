...
describe('when generating ods and comparing it to data standard 3.0 authoritative artifacts for ODS/API 5.0 in Alliance Mode', (): void => {
  ...
  it('should have core with no differences', async () => {
    expect(coreResult).toBeDefined();
    const gitCommand = `git diff --shortstat --no-index --ignore-space-at-eol -- ${authoritativeCoreOds} ${generatedCoreOds}`;
...
describe('when generating ods with simple extensions and comparing it to data standard 3.0 authoritative artifacts for ODS/API 5.0 in Alliance mode', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v5_AllianceMode/');
...
describe('when generating ods with student transcript extensions and comparing it to data standard 3.0 authoritative artifacts for ODS/API 5.0 in Alliance mode', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v5_AllianceMode/');
