...
import { EnhancerResult, MetaEdEnvironment, Domain, Subdomain, DomainItem, getAllEntitiesOfType } from '@edfi/metaed-core';
...
export function enhance(metaEd: MetaEdEnvironment): EnhancerResult {
  ...
  allDomains.forEach((domain: Domain | Subdomain) => {
    domain.domainItems.forEach((domainItem: DomainItem) => {
      // Skip if the referenced entity is not available (could be validation error)
      if (!domainItem.referencedEntity) return;
      ...
      // Skip if the entity doesn't have the edfiOdsApi data section initialized
