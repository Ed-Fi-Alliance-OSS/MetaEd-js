...
import { enhance } from '../../src/diminisher/DisciplineActionHardcodedSecurityDiminisher';
...
describe('when diminishing DisciplineAction with missing resource', (): void => {
  const metaEd: MetaEdEnvironment = newMetaEdEnvironment();
  metaEd.plugin.set('edfiApiSchema', newPluginEnvironment());
  ...
  it('should throw error', () => {
    expect(() => runEnhancers(metaEd)).toThrow(
      `DisciplineActionHardcodedSecurityDiminisher: Fatal Error: 'DisciplineAction' not found in EdFi Data Standard 8.0`,
    );
  });
});

describe('when diminishing DisciplineAction with invalid json path', (): void => {
  const metaEd: MetaEdEnvironment = newMetaEdEnvironment();
  metaEd.plugin.set('edfiApiSchema', newPluginEnvironment());
  metaEd.dataStandardVersion = '8.0';
  const namespaceName = 'EdFi';
  ...
  it('should throw error', () => {
    expect(() => runEnhancers(metaEd)).toThrow(
      `DisciplineActionHardcodedSecurityDiminisher: Fatal Error: EdFi Data Standard 8.0 has removed ResponsibilitySchool resource from 'DisciplineAction'`,
    );
  });
});

describe('when diminishing DisciplineAction with with missing property', (): void => {
  const metaEd: MetaEdEnvironment = newMetaEdEnvironment();
  metaEd.plugin.set('edfiApiSchema', newPluginEnvironment());
  metaEd.dataStandardVersion = '8.0';
  const namespaceName = 'EdFi';
  ...
  it('should throw error', () => {
    expect(() => runEnhancers(metaEd)).toThrow(
      `DisciplineActionHardcodedSecurityDiminisher: Fatal Error: EdFi Data Standard 8.0 has removed ResponsibilitySchoolId property from 'DisciplineAction'`,
    );
  });
});
