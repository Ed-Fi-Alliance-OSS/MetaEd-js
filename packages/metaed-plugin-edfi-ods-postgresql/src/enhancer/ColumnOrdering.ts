...
export function getAlternateKeys(table: Table): Column[] {
  if (
    ...
    const alternativeKeyColumns = table.columns.filter(
      (x) => x.isPartOfAlternateKey && x.columnId.toLowerCase() !== 'namespace',
    );
    return orderByPath(['data', 'edfiOdsPostgresql', 'columnName'])(namespaceColumn.concat(alternativeKeyColumns));
  }
