...
export async function generate(metaEd: MetaEdEnvironment): Promise<GeneratorResult> {
  ...
  if (versionSatisfies(targetTechnologyVersion, '>=7.1.0')) {
    ...
    metaEd.namespace.forEach((namespace) => {
      ...
      if (tables.length > 0) {
        ...
        results.push({
          name: 'ODS Update PostgreSQL EducationOrganization Authorization Indexes Generator',
  ...
  return {
    generatorName: 'edfiOdsPostgresql.UpdateEducationOrganizationAuthorizationIndexesGenerator',
