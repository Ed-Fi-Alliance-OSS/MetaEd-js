...
describe('when generating ods and comparing it to data standard 6.0 authoritative artifacts for ODS/API 7.3', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v7.3_ds6.0/');
  ...
  beforeAll(async () => {
    ...
    coreResult = R.head(
      R.head(state.generatorResults.filter((x) => x.generatorName === 'edfiOdsPostgresql.OdsGenerator')).generatedOutput,
    );
...
describe('when generating CreateEducationOrganizationAuthorizationIndexes and comparing it to data standard 6.0 authoritative artifacts for ODS/API 7.3', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v7.3_ds6.0/');
  const outputDirectory = `${artifactPath}`;
  let coreResult: GeneratedOutput;
  let coreFileBaseName: string;
...
describe('when generating UpdateEducationOrganizationAuthorizationIndexes and comparing it to data standard 6.0 authoritative artifacts for ODS/API 7.3', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v7.3_ds6.0/');
  const outputDirectory = `${artifactPath}`;
  let coreResult: GeneratedOutput;
  let coreFileBaseName: string;
...
describe('when generating AggregateIdColumns and comparing it to data standard 6.0 authoritative artifacts for ODS/API 7.3', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v7.3_ds6.0/');
  const outputDirectory = `${artifactPath}`;
  let coreResult: GeneratedOutput;
  let coreFileBaseName: string;
  ...
  beforeAll(async () => {
    ...
    coreResult = R.head(
      R.head(state.generatorResults.filter((x) => x.generatorName === 'edfiOdsPostgresql.AggregateIdColumnGenerator'))
        .generatedOutput,
    );
    coreFileBaseName = path.basename(coreResult.fileName, '.sql');
    generatedCoreOds = `${outputDirectory}/${coreFileBaseName}-generated.sql`;
...
describe('when generating ods with simple extensions and comparing it to data standard 6.0 authoritative artifacts for ODS/API 7.3', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v7.3_ds6.0/');
  ...
  beforeAll(async () => {
    ...
    const generatorResult: GeneratorResult = R.head(
      state.generatorResults.filter((x) => x.generatorName === 'edfiOdsPostgresql.OdsGenerator'),
    );
...
describe('when generating ods with student transcript extensions and comparing it to data standard 6.0 authoritative artifacts for ODS/API 7.3', (): void => {
  const artifactPath: string = path.resolve(__dirname, './artifact/v7.3_ds6.0/');
