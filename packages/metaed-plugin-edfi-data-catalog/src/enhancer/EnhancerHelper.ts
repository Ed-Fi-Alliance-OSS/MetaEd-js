...
import { EdFiDataCatalogRepository } from '../model/EdFiDataCatalogRepository';
...
export function pluginEnvironment(metaEd: MetaEdEnvironment): PluginEnvironment | undefined {
  return metaEd.plugin.get('edfiDataCatalog') as PluginEnvironment | undefined;
}

export function edfiHandbookRepositoryForNamespace(
  metaEd: MetaEdEnvironment,
  namespace: Namespace,
): EdFiDataCatalogRepository | null {
  const plugin: PluginEnvironment | undefined = pluginEnvironment(metaEd);
  // if plugin not there, something's very wrong
  if (plugin == null) return null;
  const handbookRepository: EdFiDataCatalogRepository | undefined = plugin.namespace.get(namespace);
