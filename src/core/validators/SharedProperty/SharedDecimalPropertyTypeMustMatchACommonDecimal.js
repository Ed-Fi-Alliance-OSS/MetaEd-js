...
export function validatable(ruleContext: any): ValidatableResult {
  const validatorName = 'SharedDecimalPropertyTypeMustMatchACommonDecimal';
  const invalidPath: ?string[] = exceptionPath(['sharedPropertyType', 'ID'], ruleContext);
  if (invalidPath) return { invalidPath, validatorName };
  return { validatorName };
}

const validationRule = errorRuleBase(validatable, validForShared(SymbolTableEntityType.commonDecimal()), failureMessageForShared('common decimal'));
